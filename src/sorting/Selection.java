...
import CDArchiveProject.RecordsStorage;
    ...
    public static void sort(List<CDRecord> records){
        for (int index = 0; index < records.size() - 1 ; index++ ) {
            ...
            for (int current_index = index + 1; current_index < records.size(); current_index++) {
                //if (records.get(current_index).getTitle() < records.get(smallest_index).getTitle()) {
                CDRecord a = records.get(current_index);
                CDRecord b = records.get(smallest_index);
